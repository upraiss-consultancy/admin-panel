...
function AllRides() {
  ...
  const handleUpdateFareValue = (total_price) => {
    ...
    let percentageOf20 = total_price * 0.2;
    let percentageOf82 = percentageOf20 * 0.82;
    setFare((prevState) => ({
      ...prevState,
      totalPrice: total_price.toFixed(2),
      driverCharge: (total_price * 0.64).toFixed(2),
      travelAllowance: (total_price * 0.16).toFixed(2),
      platformFee: percentageOf82.toFixed(2),
      gst: (percentageOf20 * 0.18).toFixed(2),
    }));
  ...
  const handleTravelAllowanceChange = (e) => {
    ...
    setFare((prevState) => ({
      ...
      driverCharge: value - Number(e.target.value),
    }));
  };
                        ...
                        <Select {...field} className="w-full" label="Packages">
                          {packages?.map((data) => (
                            <MenuItem
                              value={data?._id}
                              onClick={() => {
                                handleUpdateFareValue(data?.total * totalDays);
                                setDisabled(false);
                                setTotalBaseValue(data?.total * totalDays);
                                setValue("package_id", data?._id);
                              }}
                            >
                              {data?.package_name}
                            </MenuItem>
